...
Composite action that runs `composer audit` against a project's `composer.lock` to detect known vulnerabilities in PHP dependencies. `composer audit` is a built-in Composer command (available since Composer 2.4) that queries the GitHub Security Advisory database and the Packagist security advisories feed — no binary download or extra installation is required.
...
- Composer ≥ 2.4 must be available on the runner. It is pre-installed on all GitHub-hosted `ubuntu-latest` runners. The action verifies the version at runtime and exits with a clear error if the requirement is not met.
